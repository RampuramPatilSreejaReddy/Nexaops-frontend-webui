...
const INTEGRATIONS = [
  {
    ...
    detail: 'Enterprise GitHub account connection (RampuramPatilSreejaReddy) with multi-repository automation',
    tag: 'Healthy',
    state: 'Account: RampuramPatilSreejaReddy (3 repos discovered)',
    age: 'Synced now',
    type: 'GitHub',
    source: 'https://github.com/RampuramPatilSreejaReddy/nexaops-test-repo',
    org: 'RampuramPatilSreejaReddy',
    authMode: 'GitHub PAT / OAuth (Scopes: repo, workflow, webhooks)',
    discoveredRepos: [
      'RampuramPatilSreejaReddy/nexaops-test-repo',
      'RampuramPatilSreejaReddy/Nexaops-frontend-webui',
      'acies-global/nexaops-api-service'
    ]
  },
  { id: '02', name: 'BigQuery', detail: 'Warehouse job telemetry', tag: 'Healthy', state: '48 jobs', age: 'Synced 2m ago', type: 'BigQuery', source: 'Data warehouse' },
  { id: '03', name: 'Apache Kafka', detail: 'Streaming health signals', tag: 'Attention', state: '2 topics', age: 'Checked 8m ago', type: 'Kafka', source: 'Event streams' }
]

function IntegrationsModule({ config }) {
  const [items, setItems] = useState(INTEGRATIONS)
  const [query, setQuery] = useState('')
  const [filtersOpen, setFiltersOpen] = useState(false)
  const [filters, setFilters] = useState({ type: 'All', status: 'All', source: 'All', sync: 'All' })
  const [draft, setDraft] = useState(filters)
  const [connectOpen, setConnectOpen] = useState(false)
  const [selected, setSelected] = useState(null)

  const rows = items.filter(item => `${item.name} ${item.detail} ${item.type}`.toLowerCase().includes(query.toLowerCase()) && (filters.type === 'All' || item.type === filters.type) && (filters.status === 'All' || item.tag === filters.status) && (filters.source === 'All' || item.source === filters.source) && (filters.sync === 'All' || (filters.sync === 'Recent' ? /now|2m/.test(item.age) : !/now|2m/.test(item.age))))
  const control = 'mt-1 w-full rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 text-xs text-slate-700 outline-none'

  return (
    <div className="min-h-full shrink-0 bg-[#f7f9fc] p-5 md:p-6">
      <div className="mx-auto flex max-w-[1500px] flex-col gap-5">
        <PageHeader config={config} onAction={() => setConnectOpen(true)}/>
        <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <Info icon={Network} title="Event throughput" value="12.8k" text="events received today"/>
          <Info icon={CheckCircle2} title="Sync success" value="99.8%" text="over the last 24 hours"/>
          <Info icon={Clock3} title="Last full sync" value="2m" text="across all services"/>
        </section>
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between gap-4 border-b border-slate-200 px-4 py-3">
            <div className="flex items-center gap-2 text-sm font-semibold text-slate-800">
              <PlugZap size={16} className="text-blue-600"/> Connected services
            </div>
            <div className="flex gap-2">
              <div className="relative hidden sm:block">
                <Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400"/>
                <input value={query} onChange={event => setQuery(event.target.value)} aria-label="Search integrations" placeholder="Search..." className="w-44 rounded-lg border border-slate-200 bg-slate-50 py-1.5 pl-8 pr-3 text-xs outline-none focus:border-blue-400"/>
              </div>
            </div>
          </div>
          <div className="divide-y divide-slate-100">
            {rows.map(item => (
              <button key={item.id} onClick={() => setSelected(item)} className="integration-row grid w-full grid-cols-[auto_minmax(220px,1.5fr)_minmax(160px,1fr)_92px_112px_32px] items-center gap-4 px-4 py-3.5 text-left transition-colors hover:bg-slate-50">
                <span className="grid h-8 w-8 place-items-center rounded-lg bg-slate-100 text-slate-500"><Database size={15}/></span>
                <span className="min-w-0">
                  <span className="block truncate text-xs font-semibold text-slate-800">{item.id} <i className="mx-1 not-italic text-slate-300">/</i> {item.name}</span>
                  <span className="mt-1 block truncate text-xs text-slate-400">{item.detail}</span>
                </span>
                <span className="truncate text-xs text-slate-500">{item.state}</span>
                <span onClick={event => event.stopPropagation()} className={`justify-self-start rounded-md border px-2 py-1 text-[11px] font-semibold ${item.tag === 'Healthy' ? badge.green : badge.amber}`}>{item.tag}</span>
                <span className="whitespace-nowrap text-xs text-slate-500">{item.age}</span>
                <ChevronRight size={16} className="text-slate-400"/>
              </button>
            ))}
            {!rows.length && <div className="py-10 text-center text-xs text-slate-400">No integrations match the selected filters.</div>}
          </div>
        </section>
      </div>
...
function Panel({ title, icon: Icon, children }) {
  return (
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <header className="flex items-center gap-2 border-b border-slate-200 bg-slate-50 px-4 py-3 text-xs font-semibold text-slate-700">
