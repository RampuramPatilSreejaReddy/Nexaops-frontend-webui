...
import { getJobChildren, getJobs, getResolution, getJobSummary, sendApprovalEmail, createCR } from '../api/jobs.js'
import Chatbot from '../components/Chatbot.jsx'
import SlaBreachDetail from '../components/SlaBreachDetail.jsx'
import { SPARK_FULL_LOGS, SPARK_PREVIEW_LOGS } from '../data/sparkDriverFailureLogs.js'
...
`
  ...
  const [connectedRepo, setConnectedRepoState] = useState(() => {
  ...
  const saveRepoChange = (newVal) => {
    const clean = (newVal || repoInput).replace(/^https?:\/\/github\.com\//, '').replace(/\/$/, '')
    if (clean) {
      localStorage.setItem('nexaops_github_repo', clean)
      setConnectedRepoState(clean)
    }
    setEditingRepo(false)
  }
  
  const setSent = async (value) => {
    if (value) {
      onApprove?.(job.name)
      try {
        await createCR(job.id, approvalMethod)
      } catch (err) {
        // Fall back gracefully if backend endpoint unavailable
      }
    }
    if (value && approvalMethod === 'github') { setGithubModal(true); return }
    if (value && approvalMethod === 'email') { setEmailModal(true); return }
    if (value && approvalMethod === 'platform') { setPlatformReview(true); return }
    setSentState(value)
  }
  ...
  useEffect(() => {
    ...
    document.body.appendChild(mount)
    const root = createRoot(mount)
    // Use real log file data for spark-driver-failure, else backend resolution logs
    const logsToShow = isSparkDemo ? SPARK_FULL_LOGS : (resolution?.log || SPARK_FULL_LOGS)
    root.render(<FullLogsModal job={job} logs={logsToShow} onClose={() => setFullLogsModal(false)}/>)
    return () => { root.unmount(); mount.remove() }
  }, [fullLogsModal, job, resolution, isSparkDemo])
  ...
  >
    ...
    Jira: {job.jira_ticket?.key || (isSparkDemo ? 'JIRA-SPARK-01' : 'JIRA-1023')} ↗
  </a>
  <span className="text-[10px] text-slate-400 font-mono">Incident ID: {(job.id === 'SPARK-LIVE-001' || job.name === 'spark-driver-failure') ? 'INC-2026-SPARK-01' : (job.incident_id || `INC-${job.id?.slice(0, 8)}`)}</span>
  <button onClick={handleGoBack} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3.5 py-2 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700 cursor-pointer"><ArrowLeft size={14}/>Back to {originLabel}</button>
</div></header><div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_270px]"><main className="space-y-4"><section className="overflow-hidden rounded-xl border border-slate-800 bg-[#071421] shadow-sm"><header className="flex items-center justify-between px-4 pt-3"><b className="text-[10px] uppercase tracking-wide text-slate-300">Error Logs</b><button onClick={() => setFullLogsModal(true)} className="rounded-md border border-slate-600 px-2 py-1 text-[10px] font-semibold text-slate-200 hover:bg-slate-800 transition cursor-pointer">View Full Logs</button></header><pre className="overflow-auto px-4 pb-4 pt-2 font-mono text-[10px] leading-5 text-slate-200">{logsPreview.map((l,i) => <React.Fragment key={i}>{l.ts}  [{l.svc}] <span className={l.level === 'ERROR' || l.level === 'FATAL' ? 'text-red-400' : ''}>{l.level}</span> {l.msg}{'\n'}</React.Fragment>)}</pre></section><section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><header className="flex items-center justify-between"><b className="text-[11px] font-bold uppercase tracking-wide text-slate-700">AI Resolution Workflow</b><div className="flex items-center gap-2"><span className="rounded bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-600">{resolution?.rca?.confidence != null ? `${resolution.rca.confidence}% Confidence` : (isSparkDemo ? '96% Confidence' : '—')}</span><button onClick={() => fetchResolution(true)} disabled={resolutionLoading} title="Re-run AI analysis" className="inline-flex items-center gap-1 rounded border border-slate-200 px-2 py-1 text-[10px] font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50"><RefreshCw size={11} className={resolutionLoading ? 'animate-spin' : ''}/>Reload</button></div></header><div className="mt-3 grid gap-4 border-t border-slate-100 pt-3 sm:grid-cols-2"><Info label="Root cause" text={resolution?.rca?.root_cause || (isSparkDemo ? sparkDefaultRootCause : (resolutionLoading ? 'Analyzing log trace…' : 'No AI analysis available.'))}/><Info label="AI recommendation" text={resolution?.rca?.resolution || (isSparkDemo ? sparkDefaultResolution : '')}/></div></section><CodeComparison aiCode={aiCode} productionCode={effectiveProdCode} onCompare={() => setView('compare')} onEdit={() => setView('edit')}/><section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
  <header className="flex items-center justify-between flex-wrap gap-2">
    <div className="flex items-center gap-2">
      <Github size={17} className="text-slate-700"/>
      <div>
        <b className="text-[11px] font-bold uppercase tracking-wide text-slate-700">Git Integration & Target Repository</b>
        <p className="mt-0.5 text-[10px] text-slate-400">Pull request automation binds to your connected GitHub account</p>
      </div>
    </div>
    <div className="flex items-center gap-2">
      <span className="rounded-full bg-emerald-50 border border-emerald-200 px-2.5 py-1 text-[10px] font-semibold text-emerald-700">✓ GitHub Connected</span>
      <button onClick={() => setEditingRepo(!editingRepo)} className="text-[11px] font-semibold text-blue-600 hover:underline cursor-pointer">
        {editingRepo ? 'Done' : 'Change Target Repo'}
      </button>
    </div>
  </header>
  ...
  {editingRepo ? (
    <div className="mt-3 rounded-lg border border-blue-200 bg-blue-50/60 p-3">
      <label className="block text-xs font-semibold text-slate-700">Select or Enter Target Repository</label>
      <div className="mt-2 flex gap-2">
        <input
          value={repoInput}
          onChange={e => setRepoInput(e.target.value)}
          placeholder="owner/repository (e.g. RampuramPatilSreejaReddy/nexaops-test-repo)"
          className="flex-1 rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-xs text-slate-800 outline-none focus:border-blue-500"
        />
        <button onClick={() => saveRepoChange(repoInput)} className="rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-blue-700 cursor-pointer">
          Save Repo
        </button>
      </div>
      <div className="mt-2 flex gap-2">
        <span className="text-[10px] text-slate-500">Quick presets:</span>
        <button onClick={() => saveRepoChange('RampuramPatilSreejaReddy/nexaops-test-repo')} className="text-[10px] text-blue-600 hover:underline cursor-pointer">nexaops-test-repo</button>
        <span className="text-[10px] text-slate-300">·</span>
        <button onClick={() => saveRepoChange('RampuramPatilSreejaReddy/Nexaops-frontend-webui')} className="text-[10px] text-blue-600 hover:underline cursor-pointer">Nexaops-frontend-webui</button>
      </div>
    </div>
  ) : (
    <div className="mt-4 grid gap-4 border-y border-slate-100 py-4 sm:grid-cols-3">
      <GitDetail label="Target Repository" value={connectedRepo} href={repoUrl}/>
      <GitDetail label="Base branch" value="main"/>
      <GitDetail label="Compare branch" value="feature/CR-1023-ai-fix"/>
      <GitDetail label="Pull request" value="#42" href={`${repoUrl}/pulls`}/>
      <GitDetail label="Commit (latest)" value="9de4f21" href={`${repoUrl}/commits`}/>
      <GitDetail label="Files changed" value="1 file"/>
    </div>
  )}

  <GitProgress/>
  <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
    <span className="text-[10px] text-slate-500">Created 29-Jul-2026 03:45 PM</span>
    <button onClick={() => setPullRequestOpened(true)} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-3.5 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-blue-700 cursor-pointer">
      <GitPullRequest size={14}/>{pullRequestOpened ? 'Pull Request Opened' : 'Open Pull Request'}
    </button>
  </div>
</section><section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><header><b className="text-[11px] font-bold uppercase tracking-wide text-slate-700">Approval Method</b><p className="mt-1 text-[11px] text-slate-500">Select how you want to review and approve the AI-generated fix.</p></header><div className="mt-4 grid gap-3 md:grid-cols-3">{methods.map(([key,Icon,label,description]) => <label key={key} className={`cursor-pointer rounded-lg border p-3 transition ${approvalMethod === key ? 'border-blue-400 bg-blue-50/60 ring-1 ring-blue-100' : 'border-slate-200 hover:border-slate-300'}`}><input className="sr-only" type="radio" name="approval-method" checked={approvalMethod === key} onChange={() => setApprovalMethod(key)}/><span className="flex items-start gap-2"><span className={`mt-0.5 grid h-4 w-4 place-items-center rounded-full border ${approvalMethod === key ? 'border-blue-600' : 'border-slate-300'}`}>{approvalMethod === key && <span className="h-2 w-2 rounded-full bg-blue-600"/>}</span><Icon size={16} className={approvalMethod === key ? 'text-blue-600' : 'text-slate-500'}/><span><b className="block text-[11px] text-slate-700">{label}</b><span className="mt-1 block text-[10px] leading-4 text-slate-500">{description}</span></span></span></label>)}</div><div className="mt-4 flex items-center justify-end gap-2 border-t border-slate-100 pt-3"><Action label={sent ? 'Approval Started' : 'Approve & Start CR'} icon={CheckCircle2} onClick={() => setSent(true)}/><button onClick={() => setSent(true)} className="rounded-lg bg-blue-600 px-5 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-blue-700">Continue</button></div>{sent && <p className="mt-3 rounded-md bg-emerald-50 px-3 py-2 text-[11px] font-medium text-emerald-700">{approvalMethod === 'github' ? 'GitHub pull request workflow selected.' : approvalMethod === 'platform' ? 'In-platform review workflow selected.' : 'Email approval workflow selected.'}</p>}</section></main><aside className="h-fit overflow-hidden rounded-xl border border-slate-200 bg-white text-[11px] shadow-sm"><div className="p-4"><Summary title="Job Summary" rows={[["Start Time",startedAt],["Duration",job.runtime || '38m 24s']]}/><Summary title="Business Impact" rows={[["Impacted", (resolution?.rca?.impacted || ["spark-driver-failure", "Spark Cluster Orchestration", "Oracle Cloud HDFS Storage"]).join(', ')],["Business Impact", resolution?.rca?.business_impact || "Data Engineering pipeline stalled. Downstream ETL jobs blocked until OCI dependency is resolved."]]}/><Summary title="AI Usage" rows={[
...
function GitDetail({ label, value, href }) {
  return (
    <div>
      <div className="text-[9px] font-bold uppercase tracking-wide text-slate-400">{label}</div>
      {href ? (
        <a href={href} target="_blank" rel="noreferrer" className="mt-1 block text-[11px] font-semibold text-blue-600 hover:underline">
          {value}
        </a>
      ) : (
...
function GitHubPullRequestModal({ targetRepo = 'RampuramPatilSreejaReddy/Nexaops-frontend-webui', repoUrl, onClose }) {
  const pullRequestUrl = repoUrl ? `${repoUrl}/pulls` : 'https://github.com/RampuramPatilSreejaReddy/Nexaops-frontend-webui/pulls'
  const [copied,setCopied] = useState(false)
...
export default function JobStatus({ originPage = 'jobs', onNavigate, onApprove, resolutionCache }) {
  ...
  // Eagerly initialize job from URL param so FailedJobDetails renders immediately
  const initialJob = routeJobId ? normalizeJob((() => {
  ...
  const [jobs,setJobs] = useState(() => []), [filter,setFilter] = useState('all'), [team,setTeam] = useState('All Teams'), [environment,setEnvironment] = useState('All Environments'), [period,setPeriod] = useState(''), [dateOpen,setDateOpen] = useState(false), [startDate,setStartDate] = useState(''), [endDate,setEndDate] = useState(''), [appliedRange,setAppliedRange] = useState(null), [query,setQuery] = useState(''), [job,setJob] = useState(initialJob), [jobsLoaded,setJobsLoaded] = useState(false), [chatOpen,setChatOpen] = useState(false), [expandedIds, setExpandedIds] = useState({}), [childrenById, setChildrenById] = useState({})
  ...
  return <div className="jobs-page flex min-h-full shrink-0 flex-col bg-[#fbfcff] px-7 py-6 md:px-9">
    ...
    {routeJobId && !job && <div className="fixed inset-0 z-40 flex items-center justify-center bg-[#f6f8fc]"><div className="flex items-center gap-3 text-sm text-slate-500"><svg className="h-5 w-5 animate-spin text-blue-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>Loading incident…</div></div>}
