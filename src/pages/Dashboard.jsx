...
export default function Dashboard() {
  ...
  const srPct = parseFloat(successRate) || 99.4
  const recMins = parseInt(recovery) || 14
  const kpis = [
    { label: 'Success Rate', value: successRate, delta: '1.2% vs yesterday', icon: Activity, tone: 'green', health: srPct >= 98 ? 'green' : 'amber' },
    { label: 'Active Workflows', value: filters.team === 'All' && filters.workflowType === 'All' ? activeWorkflows : filteredCpuRows.length, delta: '8 vs yesterday', icon: Layers3, tone: 'blue', health: 'blue' },
    { label: 'Mean Recovery Time', value: recovery, delta: '3m vs yesterday', icon: Clock3, tone: 'violet', invertDelta: true, health: recMins <= 15 ? 'green' : recMins <= 30 ? 'amber' : 'red' },
    { label: 'Incidents', value: filters.status === 'All' ? incidents : filteredSlaRows.length, delta: '3 vs yesterday', icon: AlertTriangle, tone: 'red', invertDelta: true, health: incidents === 0 ? 'green' : incidents <= 3 ? 'amber' : 'red' },
    { label: 'SLA Compliance', value: '96.7%', delta: '0.8% vs yesterday', icon: ShieldCheck, tone: 'amber', health: 96.7 >= 98 ? 'green' : 96.7 >= 95 ? 'amber' : 'red' },
  ]
...
function KpiCard({ label, value, delta, icon: Icon, tone, invertDelta, health }) {
  ...
  const isUp = tone === 'green' || tone === 'blue' || tone === 'violet'
  const isGood = invertDelta ? !isUp : isUp
  const deltaTone = isGood ? 'text-emerald-500' : 'text-red-500'
  const healthBorder = { green: '#22c55e', blue: '#3b82f6', amber: '#f59e0b', red: '#ef4444' }[health]
  return <article className="rounded-lg border border-slate-200/90 bg-white p-4 shadow-sm shadow-slate-200/40" style={healthBorder ? { borderLeft: `4px solid ${healthBorder}` } : undefined}><div className="flex items-center gap-3"><span className={`grid h-11 w-11 place-items-center rounded-xl ${tones[tone]}`}><Icon size={22}/></span><div><div className="text-[10px] font-bold text-slate-600">{label}</div><div className="mt-1 font-mono text-2xl font-semibold tracking-tight text-slate-900">{value}</div><div className={`mt-1 text-[10px] font-medium ${deltaTone}`}>{isUp ? '↑' : '↓'} {delta}</div></div></div></article>
}
...
function ResourceCard({ cpuRows }) { const cpu = Math.min(94, Math.max(42, Math.round(Number(cpuRows[0]?.cpu_pct) || 42))); const gc = v => v >= 80 ? '#ef4444' : v >= 60 ? '#f59e0b' : '#22c55e'; return <Panel title="Resource Utilization" icon={Cpu}><div className="grid grid-cols-4 gap-2"><Gauge label="CPU" value={cpu} color={gc(cpu)} icon={Cpu}/><Gauge label="Memory" value={63} color={gc(63)} icon={Database}/><Gauge label="Storage" value={58} color={gc(58)} icon={Database}/><Gauge label="Network" value={34} color={gc(34)} icon={Network}/></div><button className="mt-5 text-[11px] font-semibold text-blue-600">View all resources →</button></Panel> }
